...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  pending:     { bg: "bg-orange-500/10", text: "text-orange-300",  border: "border-orange-500/20" },
  funded:      { bg: "bg-blue-500/10",   text: "text-blue-300",    border: "border-blue-500/20" },
  in_progress: { bg: "bg-amber-500/10",  text: "text-amber-300",   border: "border-amber-500/20" },
  delivered:   { bg: "bg-sky-500/10",    text: "text-sky-300",     border: "border-sky-500/20" },
  disputed:    { bg: "bg-red-500/10",    text: "text-red-300",     border: "border-red-500/20" },
  released:    { bg: "bg-green-500/10",  text: "text-green-300",   border: "border-green-500/20" },
  refunded:    { bg: "bg-zinc-500/10",   text: "text-zinc-300",    border: "border-zinc-500/20" },
  cancelled:   { bg: "bg-zinc-500/10",   text: "text-zinc-300",    border: "border-zinc-500/20" },
};
  ...
  async function fetchAll() {
    ...
    try {
      const [statsRes, txRes] = await Promise.all([
        ...
        apiFetch("/api/admin/escrow/transactions"),
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#181C23] border border-[#262B36] rounded-2xl p-4">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wide">Total Revenue</p>
          <p className="text-white text-xl font-bold">KES {(stats.totalRevenue / 100).toLocaleString()}</p>
        </div>
        <div className="bg-[#181C23] border border-[#262B36] rounded-2xl p-4">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wide">Funds Released</p>
          <p className="text-emerald-400 text-xl font-bold">KES {(stats.totalFundsReleased / 100).toLocaleString()}</p>
        </div>
        <div className="bg-[#181C23] border border-[#262B36] rounded-2xl p-4">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wide">Completed Transactions</p>
          <p className="text-white text-xl font-bold">{stats.completedTransactions.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search by creator, brand, or campaign title..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full pl-10 pr-4 py-2.5 bg-[#1A1F29] border border-[#2B3240] rounded-xl text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
        />
      ...
      <div className="space-y-3">
        {filteredTransactions.length === 0 && (
          <p className="text-center text-zinc-500 py-10">No transactions found.</p>
        )}
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                {/* CREATOR (SELLER) NAME — prominent */}
                <p className="text-white font-semibold text-base truncate">
                  {tx.sellerName}
                </p>
                <p className="text-zinc-500 text-xs mt-0.5 truncate">{tx.sellerEmail}</p>
                ...
                <p className="text-zinc-600 text-xs mt-1">
                  Brand: <span className="text-zinc-400">{tx.buyerName}</span>
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="bg-[#1B2029] rounded-lg p-3">
                <p className="text-zinc-500 text-xs mb-1">Total Held</p>
                <p className="text-white font-bold text-sm">
                  KES {(tx.totalAmount / 100).toLocaleString()}
                </p>
              </div>
              ...
              <div className="bg-[#1B2029] rounded-lg p-3">
                ...
                <p className="text-emerald-400 font-bold text-sm">
                  KES {(tx.sellerAmount / 100).toLocaleString()}
                </p>
              </div>
              ...
              <div className="bg-[#1B2029] rounded-lg p-3">
                ...
                <p className="text-orange-400 font-bold text-sm">
                  KES {(tx.feeAmount / 100).toLocaleString()}
                </p>
              </div>
            ...
            {tx.disputeReason && (
              <div className="mb-3 px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg">
                <p className="text-red-400 text-xs">
                  ⚠️ Dispute: {tx.disputeReason}
                </p>
              </div>
            )}

            {/* AUTO-RELEASE countdown if delivered */}
            {tx.autoReleaseAt && tx.status === "delivered" && (
              <div className="mb-3 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <p className="text-amber-400 text-xs">
                  ⏳ Auto-releases on {new Date(tx.autoReleaseAt).toLocaleDateString()}
                </p>
              </div>
            )}
...
function StatusBadge({ status }: { status: string }) {
  const config = STATUS_COLORS[status] ?? STATUS_COLORS["cancelled"];
      ...
      >
        ...
        <div className="p-6 space-y-4">
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
            <p className="text-blue-300 mb-2">⚠️ Confirm Release</p>
            <p className="text-sm text-zinc-300">
              Releasing escrow for <strong>{escrow.sellerName}</strong> will transfer{" "}
              <strong>KES {(escrow.sellerAmount / 100).toLocaleString()}</strong> to their account.
            </p>
          </div>

          {escrow.title && (
            <div className="bg-[#1B2029] border border-[#2A3140] rounded-xl p-4">
              <p className="text-zinc-500 text-sm mb-1">Campaign / Job</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#1B2029] border border-[#2A3140] rounded-xl p-4">
              <p className="text-zinc-500 text-sm mb-1">Creator receives</p>
              <p className="text-emerald-400 font-bold">KES {(escrow.sellerAmount / 100).toLocaleString()}</p>
            </div>
            <div className="bg-[#1B2029] border border-[#2A3140] rounded-xl p-4">
              <p className="text-zinc-500 text-sm mb-1">Platform fee</p>
              <p className="text-orange-400 font-bold">KES {(escrow.feeAmount / 100).toLocaleString()}</p>
            </div>
          </div>
