...
import { NarrativeReportDocument } from './NarrativeReportDocument';
...
type FullReport = {
  ...
  narrativeReport?: NarrativeReport | null; // <-- ADDED
  // summary: {
  //   totalCreators: number;
  //   totalSignals: number;
  //   totalEngagement: number;
...
export function ReportLayout({ report }: { report: FullReport }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 9️⃣ Narrative Report – Full AI-generated intelligence */}
      {narrativeReport && (
...
//   engagement: number;
//   timestamp: string;
//   // metadata?: SignalMetadata; // now typed
// };
...
// // This must match the props of NarrativeReportDocument
// type NarrativeReport = {
//   title: string;
//   generatedAt: string;
//   abstract: string;
...
//   limit = 3
// ) {
//   const signalIds = classifications
//     .filter(c => c.label.toLowerCase().includes(label))
//     .map(c => c.id);
//   const filtered = signals.filter(s => signalIds.includes(s.id));
//   return filtered
//     .sort((a, b) => b.engagement - a.engagement)
