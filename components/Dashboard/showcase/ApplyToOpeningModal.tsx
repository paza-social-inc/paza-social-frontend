...
export function ApplyToOpeningModal({
  ...
  const isCreatorAccount =
    String(user?.accountType ?? "").trim().toLowerCase() === "creator";
  const applyForbidden = Boolean(user) && !isCreatorAccount;
      ...
      >
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5 space-y-5">
            {applyForbidden ? (
              <p className="rounded-lg border border-border bg-muted/40 px-3 py-3 text-sm text-muted-foreground leading-relaxed">
                Only creator accounts can apply to openings. Close this dialog and use{" "}
                <span className="font-medium text-foreground">Request collaboration</span> or a project
                proposal if you represent a brand.
              </p>
            ) : (
              <>
                <Field>
          ...
          <div className="flex shrink-0 items-center justify-end gap-2 border-t border-border bg-muted/20 px-4 py-3 sm:px-6">
            ...
            <Button
              ...
              disabled={
                applyForbidden || applyMutation.isPending || uploadingFiles || !openingId
              }
            >
